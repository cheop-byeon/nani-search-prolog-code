...
nshelp:-
  ...
  write('   teleport to a room    (ex. go to the office)'),nl,
  ...
  write('   turn something off    (ex. turn on the light)'),nl,
...
room('dining room').
room(cellar).
room(attic).           % 1. Create a room called attic.
...
location(doll,attic).                % 2
location(plate,attic).               % 2
...
drop_heavy_stuff:- have(X),heavy(X),drop(X).
drop_heavy_stuff.
cant_move:- heavy(X),have(X), write(X),write(' is too heavy cannot be moved'),nl.

teleport(Place):- have(teleporter), drop_heavy_stuff, move(Place),look, !.
teleport(_):- write("Don't have teleport"),nl.
...
time(20).
advance_time:-time(X),Y is X + 1,Y =\= 24,retract(time(X)),asserta(time(Y)).
advance_time:-time(X),Y is X + 1,Y =:= 24, Z is 0, retract(time(X)),asserta(time(Z)).
...
look :-
  ...
  write('It is '),time(X), write(X),write('00 hours.'),nl.
...
can_take(Thing) :-
  ...
  location(X,Place).
can_take(Thing):-
  ((have(Thing),write("already taken "),write(Thing),nl);(not(have(Thing)),write("There is no "),write(Thing),write(" here."),nl)),fail.
...
command([teleport,Arg]) --> noun(teleport_place,Arg).
...
verb(teleport_place,teleport) --> teleport_verb.
...
noun(teleport_place,R) --> [R], {room(R)}.
noun(teleport_place,'dining room') --> [dining,room].
